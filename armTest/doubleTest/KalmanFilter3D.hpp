...
class KF3D
{
	...
	int predict(void);
	int update(void);
	int setMeasurement(double measurementX,double measurementY);

	KF3D();
	KF3D(mat4 p, mat4 a, mat4 q, mat2 s, mat2 r, double *m);
	...
	double M[4]; //4x1 array
	double MU[2]; //2x1 array
	double H[8]; //2x4 array
	double K[8]; //4x2 array
	mat4 P;
	mat4 A;
	mat4 Q;
	mat2 S;
	mat2 R;
	double Y[2]; // x y measurement
...
KF3D::KF3D()
{
	...
	Y[0]=(double)0.00; Y[1]=(double)0.00;
}

KF3D::KF3D(mat4 p, mat4 a, mat4 q, mat2 s, mat2 r, double *m)
{
	P = p;
	A = a;
	Q = q;
...
int KF3D::setMeasurement(double measurementX,double measurementY)
{
	Y[0] = measurementX;
	Y[1] = measurementY;
	return 1;
...
{
	/*M = A * M */
	double tempM[4] = {M[0],M[1],M[2],M[3]};
...
int KF3D::task4(void)
{
	double F1 = pow((double)(M[0] - S.getA1()), (double)2) + pow((M[1] - S.getB1()), 2);
	double F2 = pow((M[0] - S.getA2()), 2)  +  pow((M[1] - S.getB2()), 2);
